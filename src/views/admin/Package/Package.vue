<template>
   <div id="packages">
       ...
       <div class="add-packages">
           <router-link to="/dashboard/addpackages">
        ...
        <div class="field">
          <div for="entries">Show:
             <select  name="entries" id="entries" v-model="tableData.length" @change="getAllPackages()">
          ...
          <div class="search">
              <input type="text" v-model="tableData.search" placeholder="Search Package" @input="getAllPackages()">
          </div>
        </div>

       <datatable :columns="columns" :sortKey="sortKey" :sortOrders="sortOrders" @sort="sortBy">
           <tbody>
            <tr v-show="packages.length" v-for="(package) in packages" :key="package.id">
                <!-- <td>{{ index + 1 }}</td> -->
                <td>{{ package.id }}</td>
                <td>{{ package.food }}</td>
                <td>{{ package.chef_name }}</td>
                <td>{{ package.time }}</td>
                <td>{{ package.program_name}}</td>
                <td>{{ package.decoration }}</td>
                <td>{{ coupon.price }}</td>
                <td>{{ coupon.status}}</td>
                <td colspan="2">
